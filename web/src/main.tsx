          ...
          {import.meta.env.VITE_GOOGLE_CLIENT_ID && (
            <>
              <button
                ...
                onClick={() => {
                  // Google One Tap / Identity Services callback
                  const win = window as Window & typeof globalThis & {
                    google?: { accounts: { id: { initialize: (cfg: unknown) => void; prompt: () => void } } };
                  };
                  if (!win.google?.accounts?.id) {
                    setAuthError("Google login script not loaded. Refresh and try again.");
                    return;
                  }
