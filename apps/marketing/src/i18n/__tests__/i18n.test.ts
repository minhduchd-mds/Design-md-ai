...
describe("translate", () => {
  it("returns the Vietnamese string for the vi locale", () => {
    expect(translate("vi", "hero.ctaPrimary")).toBe("Dùng thử miễn phí");
    expect(translate("vi", "pricing.free.name")).toBe("Miễn phí");
  });

  it("returns the English string for the en locale", () => {
    expect(translate("en", "hero.ctaPrimary")).toBe("Start free");
    expect(translate("en", "pricing.free.name")).toBe("Free");
  });

  it("falls back to en when the key is missing in the requested locale", () => {
    // Force a hole in the vi dictionary for an existing key.
    const key = "hero.ctaSecondary" as TranslationKey;
    const saved = vi[key];
    try {
  ...
  it("falls back to the raw key when neither locale defines it", () => {
    const key = "footer.rights" as TranslationKey;
